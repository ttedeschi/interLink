...
def SubmitHandler():
        ...
        for container in containers:
            ...
            if container["image"].startswith("/"):
                ...
                if image_uri:
                    ...
                    logging.warning(
                        "image-uri annotation not specified for path in remote filesystem"
                    )
            else:
            ...
            if "command" in container.keys() and "args" in container.keys():
                ...
                singularity_command = (
                    commstr1 + envs + local_mounts + [image] + container["command"]
                )
            else:
    ...
    try:
        with open(
            InterLinkConfigInst["DataRootFolder"] + pod["metadata"]["name"] + ".jid",
            "r",
        ) as f:
            jid = f.read()
...
def StatusHandler():
    ...
    try:
        with open(
            InterLinkConfigInst["DataRootFolder"] + req["metadata"]["name"] + ".jid",
            "r",
        ) as f:
            jid_job = f.read()
        this_resp = {}
