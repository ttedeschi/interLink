	...
	if timeNow.Sub(timer) >= time.Second*10 {
		...
		for _, pod := range req {
			...
			if execReturn.Stderr != "" {
				...
				log.G(Ctx).Info("JID: " + JIDs[uid].JID + " | Status: " + match + " | Pod: " + pod.Name + " | UID: " + string(pod.UID))

				switch match {
				case "CD":
					if JIDs[uid].EndTime.IsZero() {
						JIDs[uid].EndTime = timeNow
						...
						f.WriteString(JIDs[uid].EndTime.Format("2006-01-02 15:04:05.999999999 -0700 MST"))
					}
				...
				case "CG":
					if JIDs[uid].StartTime.IsZero() {
						JIDs[uid].StartTime = timeNow
						...
						f.WriteString(JIDs[uid].StartTime.Format("2006-01-02 15:04:05.999999999 -0700 MST"))
					}
					containerStatus := v1.ContainerStatus{Name: pod.Spec.Containers[0].Name, State: v1.ContainerState{Running: &v1.ContainerStateRunning{StartedAt: metav1.Time{Time: JIDs[uid].StartTime}}}, Ready: true}
					resp = append(resp, commonIL.PodStatus{PodName: pod.Name, PodUID: string(pod.UID), PodNamespace: pod.Namespace, Containers: []v1.ContainerStatus{containerStatus}})
				case "F":
					if JIDs[uid].EndTime.IsZero() {
						JIDs[uid].EndTime = timeNow
